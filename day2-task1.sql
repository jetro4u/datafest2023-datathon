...
INSERT INTO Location (Location_Name, Sensor_Id, Latitude, Longitude, Elevation, Region)
with tab as (
SELECT DISTINCT Location_Name, Sensor_Id, Latitude, Longitude, Elevation, Region,
...
SELECT 
  ...
  Sensor_Id, Latitude, Longitude, Elevation, Region
FROM tab;
...
SELECT
    ...
    cd. Pest_Issue,
    cd.  Crop_yield
FROM stage_cropdataraw cd
JOIN CropType ct ON ct.Pest_Type = cd.Pest_Type;
...
SELECT
        DISTINCT timestamp AS FullDate,
...
INSERT INTO IrrigationDimension (Irrigation_Method, Water_Source, Sensor_ID, Timestamp, Irrigation_Duration_Min)
SELECT
...
-- Populate the weather_dim table
INSERT INTO Time (Fulldate, Date, Day, Month, Quarter, Year)
SELECT
        DISTINCT timestamp AS FullDate,
    	DATE(timestamp) AS Date,
...
INSERT INTO SensorDimension (Timestamp, Sensor_Type, Temperature_F, Humidity, Soil_Moisture, Light_Intensity, Battery_Level)
SELECT DISTINCT
    Timestamp,
    Sensor_Id,
    Temperature_F,
